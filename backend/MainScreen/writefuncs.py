...
from MainScreen.models import Ledger, Account, ReceiptsInvoice, AccountRecord, Receipt, Sitewide
...
def due_payment_receipt_record_create(due_id, amount, member, family, account_serial, login_user, txn_remarks):
    ...
    try:
        last_auto_id = Receipt.objects.latest("receipt_auto_id").receipt_auto_id
        receipt_id = get_financial_year() + '-R-' + str(last_auto_id + 1)
        Receipt.objects.create(receipt_id=receipt_id, receipt_auto_id=last_auto_id + 1,
        ...
        Receipt.objects.create(receipt_id=receipt_id, receipt_auto_id=1,
                                receipt_due_id=due_id, receipt_member=member,
                                receipt_date=date, receipt_family=family,
                                receipt_amount=amount, receipt_financial_year=get_financial_year(),
...
def ledger_record_create(due_id, txn_type, amount, member, family, txn_remarks, account_serial, login_user, txn_financial_year=get_financial_year()):
    date = timezone.now()
    try:
        ...
        Ledger.objects.create(txn_id=txn_id, txn_auto_id=last_auto_id + 1, txn_due_id=due_id,
                              ...
                              txn_remarks=txn_remarks, txn_financial_year=txn_financial_year,
